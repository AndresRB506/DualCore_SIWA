...
if {$CREATE_FLOORPLAN==1} {
		open_mw_cel pre_power
		close_mw_cel top_riscv_tec_pads
		source ./scripts/create_pads_spin_1.tcl
		source ./scripts/set_pad_attributes_on_cell_pad_ring_unplaced.tcl
		puts "Se crearon las celdas fisicas para los Pads"
...
#Ahora pegamos los pines y lo sties
		...
		puts "Se corrigen las conexiones a VDD de todos los puertos y celdas"
...
#set physopt_heterogeneous_site_array true (e caso de que pongamos celdas con otra unidad de mosaico (tile)
...
#set SLIVER_SIZE [expr 4 * $CELL_HEIGHT] ;# espacio entre CORE y PADS
		set SLIVER_SIZE [expr 40 * $BASIC_TRACK] ;# espacio entre CORE y PADS
		...
		set RIGHT_SLIVER [expr 2 * $SLIVER_SIZE]; ## Aca no habra pads
		set_fp_placement_strategy -virtual_IPO on
		save_mw_cel -as pad_placement_first
		create_floorplan   -start_first_row -control_type width_and_height -core_width $CORE_WIDTH -core_height $CORE_HEIGHT \
     -core_utilization 0.8 -left_io2core $SLIVER_SIZE -bottom_io2core $SLIVER_SIZE -right_io2core $RIGHT_SLIVER -top_io2core $SLIVER_SIZE;

                get_attribute [get_die_area] bbox
		puts "Se creo el plano de piso preeliminar"
 #Para conocer area del core

#Para conocer bbox del core
                get_attribute [get_core_area] bbox
	        save_mw_cel -as pad_placement_second
                adjust_fp_floorplan -die_height 1506.005 -die_width 1520  -die_origin {0 0}
		#adjust_fp_floorplan -die_height 1506.00 -die_width 1506.00  -die_origin {0 0}
# Este ajuste final nos da exactamente la distancia que queremos
  		get_attribute [get_die_area] bbox
		adjust_fp_io_placement -side l -spacing 22.0 -offset 189
		adjust_fp_io_placement -side b -spacing 10.0 -offset 189
		adjust_fp_io_placement -side t -spacing 10.0 -offset 189
		#adjust_fp_io_placement -side b -spacing 40.0 -offset 189
		#adjust_fp_io_placement -side t -spacing 40.0 -offset 189
		puts "Se ajusta el plano de piso completo"
#Colocamos los puertos en los pads
		#source ./scripts/place_ports_at_pads.tcl;
		save_mw_cel -as pad_placement_final
		#Conectamos los rellenos en los pads sin depender de un dominio de voltaje
		source ./scripts/pad_filling.tcl
		### Luego de rellenar debemos onnectar Rellenos de PAD correctamente a sus alimentaciones respectivas
		source ./scripts/fix_vdd_fillers.tcl
		puts "Se colocan los rellenos de pad y se conectan con las PG respectivas"
		derive_pg_connection
		check_mv_design -verbose > ./reports/mv_check_pad_ring_rdy.txt

		save_mw_cel -as pad_ring_placed
		close_mw_cel pad_ring_unplaced
