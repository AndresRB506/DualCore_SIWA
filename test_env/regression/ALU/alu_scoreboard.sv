...
class alu_scoreboard extends uvm_scoreboard;
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      file=$fopen("file_scoreboard.txt", "a");
      $fdisplay(file, "Pruebas de funcionamiento de la ALU");
      if (alu_pkt.ALU_CTRL==4'h0) begin
        $fdisplay(file, " ");
        $fdisplay(file, "Prueba Equal");
        $fdisplay(file, "El dato A es= %b", dato_1S);
        $fdisplay(file, "El dato B es= %b", dato_2S);
        sc_R =  0;
    	sc_O =  0;
    	sc_N =  0;
    	sc_C =  0;
        sc_Z = (dato_1S==dato_2S)? 1'b1: 1'b0;
        $fdisplay(file, "El valor de Zero es= %b", sc_Z);
        $fdisplay(file, "---------------------------");
      end
      if (alu_pkt.ALU_CTRL==4'h1) begin 
        ...
        sc_R=resultado;
        sc_Z =  0;
        $fdisplay(file, "El valor de Negative= %b", sc_N);
        $fdisplay(file, "El valor del Carry= %b", sc_C);
        $fdisplay(file, "El valor del Overflow= %b", sc_O);
        $fdisplay(file, "El valor del Resultado= %b", sc_R);
        $fdisplay(file, "---------------------------");
      end
