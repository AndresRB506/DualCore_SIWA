...
module tec_riscv_bus #(parameter bits = 65, parameter broadcast = {3{1'b1}}) (
  ...
  wire  [bits-1:0] D_push_anlg_int;
        ...
        prll_bs_ntrfs_riscv #(.drvrs(4),.bits(bits),.id(0),.bdcst(broadcast)) mbc_bs_ntrfs (
        ...
        .D_pop(D_pop_mbc_int));
         ...
         prll_bs_ntrfs_riscv #(.drvrs(4),.bits(bits),.id(1),.bdcst(broadcast)) spi_bs_ntrfs (
        ...
        prll_bs_ntrfs_riscv #(.drvrs(4),.bits(bits),.id(2),.bdcst(broadcast)) uart_bs_ntrfs (
