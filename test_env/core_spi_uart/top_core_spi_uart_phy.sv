`define DEBUG
`timescale 1ns/10ps
`include "../../TEC_RISCV/TOP/Tec_Riscv_pads_phy_no_pg.v"
...
initial begin
  ...
  if($value$plusargs("TESTNAME=%s",test_name)) begin
    $dumpfile({"../../../sim_files/core_sim_files/phy_",test_name,"_test.vcd"});
    $dumpvars(0,Test_Top);
    out = $fopen({"Metrics_Reports/Phy_Metrics_Report_",test_name,".txt"},"w+");
...
if ($time<4000000)begin
    ...
    if((($time > 40000000) || (line == "FINISH")) && (sim_timer == 1))begin
...
task print_metrics_report();
    ...
    out = $fopen({"Metrics_Reports/Phy_Metrics_Report_",test_name,".txt"},"r+");
    while(!$feof(out)) begin
        $fgets(filler,out);
    end
    $fwrite(out,"\n");
