task reference_model(output string inst_queue[$],
   ...
   while (pc < max_pc) begin
      ...
      ///////////////////////////// Instruction Fetch ///////////////////////////////////////////////////////
$display("Debug_PC %h",144);
      instruction[7:0]   = mem[144];
      instruction[15:8]  = mem[144 + 1];
      instruction[23:16] = mem[144 + 2];
      instruction[31:24] = mem[144 + 3];
      $display("Debug_Instruction: %h",instruction);
$display("PC %h",pc);
         ...
         7'b0000011 : begin //LB, LH, LW, LBU, LHU
            ...
            case (funct3)
               3'b000 : begin //LB
                  ...
                  //comment this line if not running coremark
   ...
   $display("Data: %h",reg_file[rs2]);
   $display("Mem_addr: %h",mem_addr);
   $display("Addr: %h",addr);
   $display("Base addr: %h",reg_file[rs1]);
   $display("Imm: %h",imm);
   $display("Offset: %h",ext_imm);
               end
               default : begin
                  intr = 1;
                  mcause2[2:0] = 5;
   ...
   file = $fopen("temp.txt","w"); //this file is to know when the program has finished
   $fwrite(file,data);
   $fwrite(file,"\n");
   $fwrite(file,"0 \n");
   $fclose(file);
